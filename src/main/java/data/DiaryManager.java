...
public class DiaryManager {
    ...
    public static void addDiaryEntry(DiaryEntry entry) {
        ...
        all.add(entry);
        saveAllDiaries(all); // Backups are created automatically here
    }
    ...
    public static void saveAllDiaries(List<DiaryEntry> allEntries) {
        ...
        for (DiaryEntry e : allEntries) {
            ...
            String title = e.getTitle().replace(",", " ");   // Ensure no CSV issues
        ...
        FileUtilities.writeAllLines(DIARIES_FILE, lines); // Save to the main file

        // Backup the file after saving
        String backupDir = "backups/";
        File backupFolder = new File(backupDir);
        if (!backupFolder.exists()) {
            backupFolder.mkdir(); // Create backups folder if it doesn’t exist
        }
        String backupFileName = backupDir + "diaries_backup_" + System.currentTimeMillis() + ".csv";
        FileUtilities.copyFile(DIARIES_FILE, backupFileName); // Copy file
    }


    public static void deleteDiaryEntry(DiaryEntry entry) {
        List<DiaryEntry> all = loadAllDiaries();
        all.removeIf(e -> e.getOwnerUsername().equals(entry.getOwnerUsername()) &&
                e.getTitle().equals(entry.getTitle()) &&
                e.getCreatedDate().equals(entry.getCreatedDate()));
        saveAllDiaries(all); // Backups created here too
    }
